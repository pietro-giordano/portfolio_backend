                                    ...
                                    <li class="flex justify-between items-center gap-x-6 p-5">
                                          <div class="flex gap-x-4">
                                                <div class="min-w-0 flex-auto">
                                                      <p class="text-xl leading-6 text-gray-900">From: <span class="font-semibold">{{ $message->name }}</span></p>
                                                      <p class="mt-1 truncate text-xs leading-5 text-gray-500">At {{ $message->created_at }}</p>
                                                </div>
                                          </div>
                                          ...
                                          <div x-data="{ open: false }" @click.outside="open = false" @close.stop="open = false" class="flex justify-end :items-center gap-x-4 relative">
                                                <a href="{{ route('admin.messages.show', $message->id) }}" class="border border-gray-300 py-1 px-2 rounded-lg font-bold hover:bg-gray-100">
                                                      View message
                                                </a>
